...
import { dispatchScrollToPrompt } from './scrollToPromptEvent.js';
 ...
 * `session.promptHistory` IPC verb. Click a row → TerminalPane scrolls
 * its scrollback to the matching prompt. Refreshes on
...
export function HistoryPanel({ sessionId }: Props): JSX.Element {
  ...
  function scrollToPrompt(p: PromptEntry): void {
    // Use a short prefix as the search snippet. Long prompts get wrapped
    // by Claude's TUI across multiple rendered rows; xterm's search is
    // line-based and wouldn't match the full text. ~60 chars is short
    // enough to fit on one rendered row but long enough to be unique.
    const snippet = p.text.replace(/\s+/g, ' ').trim().slice(0, 60);
    if (!snippet) return;
    dispatchScrollToPrompt({ sessionId, snippet });
  }
    ...
    <div className="history-list">
      {ordered.map((p) => (
        <button
          ...
          onClick={() => scrollToPrompt(p)}
          title="Click to scroll the terminal to this prompt"
        >
